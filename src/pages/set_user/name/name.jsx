        ...
        return (
            <section className="setname">
                <section className="setname-top">
                    <input type="text" placeholder="输入用户名" className={this.state.activeInput} value={this.state.name} onChange={this.handleInput.bind(this)}/>
                    <div>
                        {
                            !this.state.activeText?
                            <p>用户只能修改一次(5~24字符之间)</p>:
                            <p className={this.state.activeText}>用户名长度在5到24位之间</p>
                        }
                    </div>
                </section>
                <section className="reset">
                    <button className={this.state.fontopacity} onClick={this.resetName.bind(this)}>确认修改</button>
                </section>
            </section>
        )
    }
