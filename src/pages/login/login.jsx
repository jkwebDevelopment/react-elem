...
import { setStore, getImgPath } from '../../utils/commons';
import './login.scss';
        ...
        let accountNumberLogin = (
            <form className="login-form">
                ...
                <section className="input-container">
                    {
                        this.state.showPwd?
                        <input type="text" placeholder="密码" value={this.state.password} onChange={this.handleInput.bind(this, 'password')}/>:
                        <input type="password" placeholder="密码" value={this.state.password} onChange={this.handleInput.bind(this, 'password')}/>
                    }
                    <div className={`button-switch ${this.state.shoePwd?'change-to-text':''}`}>
        ...
        return (
            <Animate transitionName="fade">
                <div className="login-container">
                    <Header title="密码登录" goBack={this.goBack.bind(this)}></Header>
                    {this.state.loginWay? verificationCodeLogin : accountNumberLogin}
                    ...
                    <div className="login-button" onClick={this.mobileLogin.bind(this)}>登录</div>
                    <Link to="/profile" className="to-forget">重置密码？</Link>
                    {this.state.hasAlert && <AlertTip logout={()=>{return false}} closeTip={this.closeTip.bind(this)} alertText={this.state.alertText}></AlertTip>}
                </div>
            </Animate>
        )
    }
